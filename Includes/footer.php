...
$escaped_keys = array_map(function ($key) use ($conn) {
    return "'" . $conn->real_escape_string($key) . "'";
}, $footer_content_keys);
$keys_string = implode(',', $escaped_keys);

$footer_result = $conn->query("SELECT setting_key, setting_value FROM site_settings WHERE setting_key IN ($keys_string)");

$footer_data = [];
if ($footer_result) {
    while ($row = $footer_result->fetch_assoc()) {
        $footer_data[$row['setting_key']] = $row['setting_value'];
    }
}
?>
